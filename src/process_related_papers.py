...
parser.add_argument("--file", type=str, default="./data/cvpr2024.csv")
...
def normalize_text(text):
    """텍스트를 정규화하는 함수: 소문자로 변환, 특수 문자 정리, 중복 공백 제거"""
    if not isinstance(text, str):
        return ""
    # 소문자로 변환
    text = text.lower()
    # LaTeX 형식 명령어 제거 (예: \textbf{})
...
related_titles = []
related_abstracts = []
related_abstract_similarities = []
related_abstract_mean_similarities = []
related_detailed_similarities = []  # 각 비교 논문별 유사도를 저장할 리스트
...
# 초록 텍스트들을 배치로 묶어서 임베딩 계산
paper_embeddings = get_embeddings(normalized_paper_list)


# 유사도 판단 기준을 위한 함수
def is_paper_related(similarities, args):
    """
    여러 기준을 종합해 논문이 관련 있는지 판단하는 함수

    1. 평균 유사도가 threshold를 넘는지
    2. 상위 k개 논문 중 일정 개수 이상이 개별 threshold를 넘는지
    3. 전체 유사도 분포 중 상위 percentile에 들어가는지
    """
    # 1. 평균 유사도 체크
...
#         .numpy()
#     )
#     all_mean_similarities.append(np.mean(abstract_similarity))

# # 상위 percentile 계산
# if len(all_mean_similarities) > 0:
#     percentile_threshold = np.percentile(
#         all_mean_similarities, 100 - args.top_percentile
#     )
#     print(f"\n상위 {args.top_percentile}% 유사도 기준값: {percentile_threshold:.4f}")
# else:
#     percentile_threshold = args.similarity_threshold

# 기존 코드를 대체하여 새로운 관련성 판단 기준 적용
for i, paper_embedding in enumerate(paper_embeddings):
    title = titles[i]
    ...
    # 여러 기준으로 관련성 판단 (함수 사용)
    is_related, reason = is_paper_related(abstract_similarity, args)

    # 또는 상위 percentile에 포함되면 관련 논문으로 판단
    # if not is_related and mean_similarity >= percentile_threshold:
    #     is_related = True
    #     reason = (
    #         f"상위 {args.top_percentile}% 유사도에 포함 (> {percentile_threshold:.4f})"
    #     )

    if is_related:
        related_titles.append(title)
        related_abstracts.append(abstract)
        related_abstract_similarities.append(abstract_similarity)
        related_abstract_mean_similarities.append(np.mean(abstract_similarity))

        # 각 비교 논문별 유사도를 저장
        detailed_similarities = {}
        for idx, (comp_title, sim) in enumerate(
            zip(compare_papers.keys(), abstract_similarity)
        ):
        ...
        sorted_similarities = {
            k: v
            for k, v in sorted(
                detailed_similarities.items(), key=lambda item: item[1], reverse=True
            )
        }
        related_detailed_similarities.append(sorted_similarities)
        ...
        print("\n가장 유사한 논문 TOP 3:")
        for idx, (comp_title, sim) in enumerate(list(sorted_similarities.items())[:3]):
...
related_paper_df = pd.DataFrame(
    {
        ...
        "abstract": related_abstracts,
        "mean_abstract_similarity": related_abstract_mean_similarities,
        "detailed_similarities": related_detailed_similarities,
    }
)
